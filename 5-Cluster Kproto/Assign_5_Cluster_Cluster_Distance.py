...
Script Goal: Take most frequent 20 diagnosis and medication and apply K-prototypes algorithm to cluster the data into 5 clusters. And assign, save the cluster labels and distances to the original data.
...
def load_data_with_top20_diag_med():
    ...
    le = LabelEncoder()
    original_df_1merged_df["tillnextepisode"] = le.fit_transform(
        pd.cut(
    ...
    print(original_df_1merged_df.columns)
    return (
...
def with_with_top20_diag_med():
    (
        ...
        original_df_Full_ICD10_ATC_backup,
        original_df_1merged_df,
    ) = load_data_with_top50_diag_med()

    categorical = [0, 1, 2, 3, 4, 5] + list(range(17, 57))
    print("******** With Diagnosis & Medication using gower distance ******** \n")
    kprototypes = KPrototypes(n_clusters=5, init="Huang", random_state=42)
    clusters = kprototypes.fit_predict(
        with_diag_medic_selected_column_merged_df_top20,
        categorical=categorical,
    )
    print(with_diag_medic_selected_column_merged_df_top20.info())
    # Assume that `kproto` is your fitted k-prototypes model# and
    ...
    merged_df.to_csv(
        "ClusterLabel_Full_ICD10_ATC_Dummies_ICD10_ATC_20.csv",
        index=False,
    )
